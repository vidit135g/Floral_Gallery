...
/** LZW encoder for GIF — public-domain port (Kevin Weiner / A. Cox / GifCompress). */
class LZWEncoder {
    ...
    LZWEncoder(int width, int height, byte[] pixels, int color_depth) {
        imgW = width; imgH = height; pixAry = pixels;
    ...
    private void cl_hash(int hsize) { for (int i = 0; i < hsize; ++i) htab[i] = -1; }

    private void compress(int init_bits, OutputStream outs) throws IOException {
        int fcode, i, c, ent, disp, hshift;
    ...
    private int MAXCODE(int n_bits) { return (1 << n_bits) - 1; }
    ...
    private void output(int code, OutputStream outs) throws IOException {
        ...
        if (code == EOFCode) {
            while (cur_bits > 0) { char_out((byte) (cur_accum & 0xff), outs); cur_accum >>= 8; cur_bits -= 8; }
            flush_char(outs);
        }
    }
}
